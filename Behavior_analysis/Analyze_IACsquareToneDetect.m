...
MedianSNR(9,strcmp(Subjects,'S132')) = nan; %S132 gave up on this trial due to exhaustion. Removing data point
figure('Position',[1, 1, 1000, 637]), hold on,%plot((0:1/fs:1.6)*1000,FittedCurve,'r','linewidth',2), hold on
boxplot((MedianSNR(2:end,:) - MedianSNR(1,:))',WindowSizes(2:end), 'Colors','k')
%xticks([1,4, 7, 9])
%xticklabels(WindowSizes([1,4, 7, 9]))
xlabel('Window Size (Sec)'), ylabel('Detection Improvement (dB)')
set(gca,'fontsize',15)
%set(findobj(gca,'type','line'),'linew',1.5)
fig = gcf;
fig.PaperUnits = 'inches';
fig.PaperPosition = [0 0 9 6];
print([Fig_path 'IACsqTone_BP'],'-dsvg')
